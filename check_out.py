...
class CheckOut:
    def __init__(self, root):
        ...
        # Prikaz informacija
        self.label = Label(top, font=('arial', 50, 'bold'), text="Check-out", fg="#CE0404", anchor="center")
                        ...
                        for i in ans:
                            if room_number1 == int(i[1]):
                                self.get_info_entry.insert(INSERT,
                                                           '\n' + str(i[0]) + ' odjavljen je iz sobe ' + str(i[1]) + '\n')
                                with conn:
                                    cursor.execute("""DELETE FROM Hotel where room_number = ?""", [room_number1])

                else:
                    self.get_info_entry.insert(INSERT, "Uneti broj sobe nije validan")

        # POST dugme
        self.check_out_button = Button(bottom, text="Odjavi gosta", font=('', 15), bg="#CE0404", relief=RIDGE, height=2,
                                       width=15,
                                       fg="white", anchor="center", command=check_out)
        self.check_out_button.grid(row=3, column=2, padx=10, pady=10)

        # Povratak u glavni meni dugme
        self.home_button = Button(bottom, text="Glavni meni", font=('', 15), bg="#CE0404", relief=RIDGE, height=2, width=15,
